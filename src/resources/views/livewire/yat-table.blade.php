            ...
            @else
                <table class="min-w-full border-collapse border {{ $themeConfig['table']['wrapper'] }}">
                    <thead
                        ...
                        @if (!empty($secondaryHeaders))
                            <tr class="{{ $themeConfig['table']['tr_thead'] }}">
                                @if ($has_counter)
                                    <th class="px-2 {{ $themeConfig['table']['th'] }}"></th>
                                @endif
                                @if ($has_bulk)
                                    <th class="px-5 {{ $themeConfig['table']['th'] }}"></th>
                                @endif
                                @foreach ($columns as $column)
                                    @if (!$column->isHidden && $column->isVisible)
                                        <th class="px-5 py-2 text-xs font-normal whitespace-nowrap {{ $themeConfig['table']['th'] }} {{ $column->th_classes }}">
                                            {!! $secondaryHeaders[$column->key] ?? '' !!}
                                        </th>
                                    @endif
                                @endforeach
                                @if ($yat_is_mobile && $hasMobileCollapsedColumns)
                                    <th class="px-5 py-2 {{ $themeConfig['table']['th'] }}"></th>
                                @endif
                            </tr>
                        @endif
                    </thead>
